...
// Map a partition to its parent disk using Disk Arbitration
static DADiskRef map_partition_to_disk(const char *path)
{
	...
	CFStringRef bsdName = CFDictionaryGetValue(desc, kDADiskDescriptionMediaBSDNameKey);
	if (CFStringHasSuffix(bsdName, CFSTR("s"))) {
		// This is a partition, get the parent disk
		CFStringRef parentPath = CFStringCreateWithFormat(kCFAllocatorDefault, NULL,
								CFSTR("/dev/rdisk%s"),
								CFStringGetCStringPtr(bsdName, kCFStringEncodingUTF8));
		DADiskRef parent = DADiskCreateFromBSDName(kCFAllocatorDefault, session, CFStringGetCStringPtr(parentPath, kCFStringEncodingUTF8));
