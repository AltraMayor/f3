...
int partition_create(const char *dev_filename, const struct partition_options *opt)
{
    ...
    /* Compute slice length in sectors.  GPT requires every user slice
     * start/end on an 8-sector boundary (4 KiB).  Trim *down* so we never
     * trespass past last_sector even after alignment.
     */
